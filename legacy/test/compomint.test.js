...
describe("Compomint Core - Template Syntax", () => {
  ...
  beforeAll(() => {
    ...
    if (!compomint) {
      throw new Error(
        "Compomint library not loaded correctly into JSDOM environment."
      );
    }
  });
  ...
  describe("templateBuilder (compomint.template)", () => {
    ...
    it("should cache the compiled template", () => {
      const renderingFunc1 = compomint.template(
        "test-cache",
        "<div>Cache</div>"
      );
  ...
  describe("Template Syntax Rules", () => {
    ...
    describe("style rule (<style id=...>)", () => {
      ...
      it("should shadow style tags without an ID", () => {
        ...
        expect(component.element.shadowRoot.children[0].outerHTML).toBe(
          "<style>div { color: blue; }</style>"
        );
        expect(component.element.shadowRoot.children[1].outerHTML).toBe(
          "<div>No ID</div>"
        );
      });

      it("should access able compomint namespace", () => {
        const templateString = `<style>div { color: ##=compomint.configs.debug ? 'red' : 'black'##; }</style><div>No ID</div>`;
        const renderingFunc = compomint.template(
          "style-compomint-ns",
          templateString
        );
        ...
        expect(component.element.outerHTML).toBe(
          "<style-compomint-ns></style-compomint-ns>"
        );
        expect(component.element.shadowRoot.children[0].outerHTML).toBe(
        ...
        expect(component2.element.shadowRoot.children[0].outerHTML).toBe(
    ...
    describe("commentArea rule (##*...##)", () => {
      ...
      it("should not execute JS inside the comment block (due to ## wrapper)", () => {
        ...
        const templateString =
          '<div>##* console.log("Should not execute"); let x = 1; ##Result</div>';
      ...
      it("should not access able compomint namespace", () => {
        expect(compomint.configs.debug).toBe(false);
        const templateString =
          '<div>##* console.log("Should not execute"); let x = 1; compomint.configs.debug = true; ##Result</div>';
        const consoleSpy = jest.spyOn(console, "log");
        const renderingFunc = compomint.template(
          "comment-no-exec",
          templateString
    ...
    describe("preEvaluate rule (##!...##)", () => {
      ...
      beforeEach(() => {
        ...
        window.preEvalMock = preEvalMock;
      });

      afterEach(() => {
        delete window.preEvalMock; // Clean up global mock
      });

      it("should execute code during template compilation", () => {
        const templateString =
          '##! window.preEvalMock("compile-time", tmplId) ##<div>Rendered</div>';
        const renderingFunc = compomint.template(
          "pre-eval-exec",
          templateString
        );
        ...
        expect(preEvalMock).toHaveBeenCalledWith(
          "compile-time",
          "pre-eval-exec"
        );
      ...
      it("should access able compomint, tmplId namespace during compilation", () => {
        const templateString = `
          ##!
          compomint.addI18ns({
            'pre-eval-compomint': {
              'tmplIdLabel': {
                'en': 'Template id is ' + tmplId,
                'ko': '템플릿 아이디는 ' + tmplId,
                'ja': 'テンプレートIDは ' + tmplId,
                'zh': '模板 ID ' + tmplId
              }
          ...
          ##
          <div>Content: ##=i18n.tmplIdLabel##</div>`;
        ...
        const component = renderingFunc({});
        expect(component.element.outerHTML).toBe(
          "<div>Content: Template id is pre-eval-compomint</div>"
        );
    ...
    describe("Element Props (data-co-props)", () => {
      it("should set a single attribute using setAttribute", () => {
        const renderingFunc = compomint.template(
          "props-single",
          '<div data-co-props="##:data.myProps##">Content</div>'
        );
      ...
      it("should set multiple attributes using setAttribute", () => {
        const renderingFunc = compomint.template(
          "props-multi",
          '<span data-co-props="##:data.attrs##">Text</span>'
        );
        const component = renderingFunc({
          attrs: {
        ...
        expect(component.element.getAttribute("aria-label")).toBe(
          "Input Field"
        );
      ...
      it("should overwrite existing attributes if specified", () => {
        ...
        const renderingFunc = compomint.template(
          "props-overwrite",
          '<button class="initial" data-co-props="##:data.newAttrs##">Btn</button>'
        );
        const component = renderingFunc({
          newAttrs: { class: "overwritten", disabled: true },
        });
      ...
      it("should handle props object defined inline", () => {
        const renderingFunc = compomint.template(
          "props-inline",
          "<div data-co-props=\"##:{ 'data-id': data.id, 'data-next-value': 100, title: 'Static' }##\">Inline</div>"
        );
      ...
      it("should work on nested elements", () => {
        const renderingFunc = compomint.template(
          "props-nested",
          '<div><span data-co-props="##:data.spanAttrs##">Nested</span></div>'
        );
      ...
      it("should access able compomint namespace", () => {
        const renderingFunc = compomint.template(
          "props-compomint-ns",
          "<div data-co-props=\"##:{ title: compomint.configs.debug ? 'Debug Mode' : 'Normal Mode' }##\">Content</div>"
        );
        const component = renderingFunc({});
        expect(component.element.getAttribute("title")).toBe("Normal Mode");
        compomint.configs.debug = true;
        const component2 = renderingFunc({});
        expect(component2.element.getAttribute("title")).toBe("Debug Mode");
      });
    });
    ...
    describe("event (data-co-event)", () => {
      it("should attach a simple click handler (function)", () => {
        ...
        const renderingFunc = compomint.template(
          "event-click",
          '<button data-co-event="##:data.clickHandler##">Click</button>'
        );
      ...
      it("should attach handlers from an event map object", () => {
        ...
        const renderingFunc = compomint.template(
          "event-map",
          '<div data-co-event="##:data.eventMap##">Hover</div>'
        );
      ...
      it('should execute "load" event immediately', () => {
        ...
        const renderingFunc = compomint.template(
          "event-load",
          '<div data-co-event="##:data.eventMap##">Load</div>'
        );
        const component = renderingFunc({ eventMap });
      ...
      it('should assign element via "namedElement" key in event map', () => {
        ...
        const renderingFunc = compomint.template(
          "event-namedElement",
          '<div data-co-event="##:data.eventMap##">Ref</div>'
        );
        const component = renderingFunc({ eventMap });
      ...
      it('should provide trigger functions via "triggerName"', () => {
        ...
        const renderingFunc = compomint.template(
          "event-trigger",
          '<button data-co-event="##:data.eventMap##">Trigger</button>'
        );
        const component = renderingFunc({ eventMap });
      ...
      it("should handle custom data (object)", () => {
        ...
        const renderingFunc = compomint.template(
          "event-custom-obj",
          '<button data-co-event="##:data.handler::data.custom##">Data</button>'
        );
      ...
      it("should handle custom data (variable)", () => {
        ...
        const renderingFunc = compomint.template(
          "event-custom-var",
          '##let customVar = {id: 101};##<button data-co-event="##:data.handler::customVar##">Data</button>'
        );
      ...
      it("should handle custom data (string)", () => {
        ...
        const renderingFunc = compomint.template(
          "event-custom-str",
          "<button data-co-event=\"##:data.handler::'test data'##\">Data</button>"
        );
        const component = renderingFunc({ handler });
        component.element.click();
        expect(handler).toHaveBeenCalledWith(
      ...
      it("should handle custom data (number)", () => {
        ...
        const renderingFunc = compomint.template(
          "event-custom-num",
          '<button data-co-event="##:data.handler::100##">Data</button>'
        );
        const component = renderingFunc({ handler });
        component.element.click();
        expect(handler).toHaveBeenCalledWith(
      ...
      it("should handle multiple handlers using :::", () => {
        ...
        const custom2 = { id: 2 };
        const renderingFunc = compomint.template(
          "event-multi-handlers",
          '<div data-co-event="##:data.h1::data.c1 ::: data.h2::data.c2##">Multi</div>'
        );
        const component = renderingFunc({
          h1: handler1,
          c1: custom1,
        ...
        expect(handler1).toHaveBeenCalledWith(
          expect.any(Event),
          expect.objectContaining({ customData: custom1 })
        );

        expect(handler2).toHaveBeenCalledTimes(1);
        expect(handler2).toHaveBeenCalledWith(
          expect.any(Event),
          expect.objectContaining({ customData: custom2 })
        );
      });
      ...
      it("should access able compomint namespace", () => {
        ...
        const renderingFunc = compomint.template(
          "event-compomint-ns",
          '<button data-co-event="##:data.clickHandler::`debug: ${compomint.configs.debug}`##">Click</button>'
        );
        const component = renderingFunc({ clickHandler });
        const button = component.element;
        ...
        expect(clickHandler).toHaveBeenCalledWith(
          ...
          expect.objectContaining({
            // context object
            data: { clickHandler },
            customData: "debug: false", // <-- customData is determined when compiled, so it does not change even if refreshed.
          })
        );
        expect(button.hasAttribute("data-co-event")).toBe(false);
      });
    ...
    describe("element (##%)", () => {
      it("should insert string content", () => {
        const renderingFunc = compomint.template(
          "el-insert-string",
          '<div>Before ##% "Inserted String" ## After</div>'
        );
        const component = renderingFunc({});
        expect(component.element.innerHTML).toBe(
          "Before Inserted String After"
        );
      });
      ...
      it("should insert HTML string (single element)", () => {
        const renderingFunc = compomint.template(
          "el-insert-html-single",
          '<div>##% "<span>HTML Span</span>" ##</div>'
        );
        const component = renderingFunc({});
      ...
      it("should insert HTML string (multiple elements)", () => {
        const renderingFunc = compomint.template(
          "el-insert-html-multi",
          '<div>##% "<i>Italic</i><b>Bold</b>" ##</div>'
        );
        const component = renderingFunc({});
      ...
      it("should insert another Compomint component", () => {
        const innerRender = compomint.template(
          "inner-comp",
          "<section>Inner Component</section>"
        );
        const innerComponent = innerRender({});
        const renderingFunc = compomint.template(
          "el-insert-component",
          "<div>Outer: ##% data.inner ##</div>"
        );
        const component = renderingFunc({ inner: innerComponent });
        expect(component.element.innerHTML).toBe(
          "Outer: <section>Inner Component</section>"
        );
        expect(component.element.querySelector("section")).toBe(
      ...
      it("should handle nonblocking element insertion (##% ... ::true)", () => {
        const renderingFunc = compomint.template(
          "el-insert-nb",
          '<div>##% "<span>NB Content</span>" :: true ##</div>'
        );
        const component = renderingFunc({});
        // Initially, the placeholder is there
        expect(
          component.element.querySelector("template[data-co-tmpl-element-id]")
        ).not.toBeNull();
        expect(component.element.innerHTML).toContain(
          '<template data-co-tmpl-element-id="0"></template>'
        );
        // Run timers
        jest.runAllTimers();
      ...
      it("should call beforeAppendTo and afterAppendTo for array items", () => {
        ...
        const item1 = {
          element: "<span>One</span>",
          beforeAppendTo: beforeHook1,
          afterAppendTo: afterHook1,
        };
        const item2 = {
          element: document.createElement("b"),
          beforeAppendTo: beforeHook2,
          afterAppendTo: afterHook2,
        };
        item2.element.textContent = "Two";

        const renderingFunc = compomint.template(
          "el-insert-array-hooks",
          "<div>##% data.items ##</div>"
        );
      ...
      it("should handle missing placeholder gracefully", () => {
        const renderingFunc = compomint.template(
          "el-insert-missing-placeholder",
          "<div>Placeholder removed</div>"
        );
        const component = renderingFunc({});

        // Manually simulate lazy execution with a placeholder ID that won't be found
        const lazyScope = {
          elementArray: [{ childTarget: "Content", nonblocking: false }],
        };
        ...
        expect(() => {
          templateConfig.rules.element.lazyExec(
            {},
            lazyScope,
            component,
            docFragment
      ...
      it("should handle invalid childTarget gracefully", () => {
        ...
        const consoleWarnSpy = jest
          .spyOn(console, "warn")
          .mockImplementation((test, test2) => {});
        renderingFunc({ invalid: { some: "object" } }); // Re-render with debug on
        expect(consoleWarnSpy).toHaveBeenCalledWith(
          expect.stringContaining("Invalid target for element insertion"),
          { some: "object" }
        );
        consoleWarnSpy.mockRestore();
        configs.debug = false;
      ...
      it("should access able compomint namespace", () => {
        const renderingFunc = compomint.template(
          "el-insert-compomint-ns",
          `<div>##% compomint.configs.debug ? 'Debug Mode' : 'Normal Mode' ##</div>`
        );
        const component = renderingFunc({});
        expect(component.element.innerHTML).toBe("Normal Mode");
        compomint.configs.debug = true;
        const component2 = renderingFunc({});
        expect(component2.element.innerHTML).toBe("Debug Mode");
      });
    });
    ...
    describe("interpolate rule (##=...##)", () => {
      it("should interpolate simple variable values", () => {
        const templateString =
          "<div>Name: ##=data.name##, Age: ##=data.age##, Active: ##=data.isActive##</div>";
      ...
      it("should render empty string for null or undefined", () => {
        const templateString =
          "<div>Null: [##=data.isNull##], Undefined: [##=data.isUndef##]</div>";
        const renderingFunc = compomint.template(
          "interp-nullish",
          templateString
        );
      ...
      it("should handle complex nested expressions", () => {
        const templateString =
          '<div>##= data.user ? data.user.name.toUpperCase() : "Guest" ##</div>';
        const renderingFunc = compomint.template(
          "interp-complex",
          templateString
        );
      ...
      it("should render tag attribute", () => {
        const templateString =
          '<input type="checkbox" ##=data.checked ? "checked" : ""##></input>';
        const renderingFunc = compomint.template(
          "interp-attribute",
          templateString
        );
        const component1 = renderingFunc({ checked: true });
        expect(component1.element.outerHTML).toBe(
    ...
    describe("namedElement (data-co-named-element)", () => {
      it("should assign element to component scope using string literal key", () => {
        const renderingFunc = compomint.template(
          "named-el-string",
          "<button data-co-named-element=\"##:'myButton'##\">Click</button>"
        );
        const component = renderingFunc({});
        ...
        expect(component.myButton.hasAttribute("data-co-named-element")).toBe(
          false
      ...
      it("should assign element to component scope using variable key", () => {
        const renderingFunc = compomint.template(
          "named-el-var",
          '## let btnKey = "theButton"; ##<span data-co-named-element="##:btnKey##">Span</span>'
        );
        const component = renderingFunc({});
        ...
        expect(component.theButton.hasAttribute("data-co-named-element")).toBe(
          false
        );
      });
      ...
      it("should not throw error if element is not found", () => {
        const renderingFunc = compomint.template(
          "named-el-missing",
          "<div><!-- Element removed before lazyExec --></div>"
        );
        const component = renderingFunc({});
        ...
        expect(() => {
          templateConfig.rules.namedElement.lazyExec(
            {},
            lazyScope,
            component,
            docFragment
      ...
      it("should access able compomint namespace", () => {
        const renderingFunc = compomint.template(
          "named-el-compomint-ns",
          `<div data-co-named-element="##:compomint.configs.debug ? 'debugDiv' : 'normalDiv'##">Content</div>`
        );
        const component = renderingFunc({});
        expect(component.normalDiv).toBeDefined();
        expect(component.debugDiv).toBeUndefined();
        compomint.configs.debug = true;
        const component2 = renderingFunc({});
        expect(component2.normalDiv).toBeUndefined();
        expect(component2.debugDiv).toBeDefined();
      });
    });
    ...
    describe("elementRef (data-co-element-ref)", () => {
      ...
      `
        ...
        expect(component.element.value).toBe("Changed in Lazy");
        expect(testFunc).toHaveBeenCalledTimes(1);
        expect(testFunc).toHaveBeenCalledWith(component.element); // Check the argument passed
        expect(component.element.hasAttribute("data-co-element-ref")).toBe(
          false
        );

        delete window.testFunc; // Clean up global
      });
      ...
      it("should call the function stored in lazyScope", () => {
        ...
        const renderingFunc = compomint.template(
          "el-ref-internal",
          '<div data-co-element-ref="##:internalRef##">Ref</div>'
        );
        const component = renderingFunc({});
        ...
        templateConfig.rules.elementRef.lazyExec(
          {},
          lazyScope,
          component,
          docFragment
    ...
    describe("elementLoad (data-co-load)", () => {
      it("should call load function with element and context", () => {
        ...
        const renderingFunc = compomint.template(
          "el-load-basic",
          '<div data-co-load="##:data.handler::data.custom##">Load Me</div>'
        );
        const component = renderingFunc({
          handler: loadHandler,
          custom: customData,
      ...
      it("should call load function without custom data", () => {
        ...
        const renderingFunc = compomint.template(
          "el-load-no-custom",
          '<span data-co-load="##:data.handler##">Load Me Too</span>'
        );
      ...
      it("should not throw error if element is not found", () => {
        ...
        // Simulate lazy execution manually
        const lazyScope = {
          elementLoadArray: [{ loadFunc: loadHandler, customData: null }],
        };
        const templateConfig = compomint.templateEngine;
        const docFragment = document.createDocumentFragment(); // Empty

        expect(() => {
          templateConfig.rules.elementLoad.lazyExec(
            { handler: loadHandler },
            lazyScope,
            component,
            docFragment
      ...
      it("should access able compomint namespace", () => {
        const loadHandler = jest.fn();
        const renderingFunc = compomint.template(
          "el-load-compomint-ns",
          `<div data-co-load="##:data.handler::compomint.configs.debug##">Load Me</div>`
        );
        const component = renderingFunc({ handler: loadHandler });
        const element = component.element;
        const parent = document.createElement("div"); // Mock parent
        ...
        expect(loadHandler).toHaveBeenCalledWith(
          ...
          expect.objectContaining({
            ...
            data: { handler: loadHandler },
            element: element,
            customData: false, // <-- customData is determined when compiled, so it does not change even if refreshed.
            component: component,
          })
        );
    ...
    describe("lazyEvaluate (###)", () => {
      it("should execute code after initial render", () => {
        ...
        const renderingFunc = compomint.template(
          "lazy-eval-basic",
          "<div>### data.handler(data.value) ##</div>"
        );
      ...
      it("should execute code with correct `this` context (single root element)", () => {
        ...
        const renderingFunc = compomint.template(
          "lazy-eval-this-single",
          "<span>### data.handler(this, data) ##</span>"
        );
        const component = renderingFunc({
          handler: lazyHandler,
          value: "test",
      ...
      it("should execute code with correct `this` context (multiple root elements -> window)", () => {
        ...
        const renderingFunc = compomint.template(
          "lazy-eval-this-multi",
          "<i>Italic</i><b>Bold</b>### data.handler(this, data) ##"
        );
        const wrapperDiv = document.createElement("div");
        const component = renderingFunc(
          { handler: lazyHandler, value: "multi" },
          wrapperDiv
        ); // Render into wrapper
      ...
      it("should execute multiple lazy blocks in order", () => {
        ...
        const renderingFunc = compomint.template(
          "lazy-eval-multi",
          "<div>### data.h1() ###<p>Middle</p>### data.h2() ##</div>"
        );
      ...
      it("should access able compomint namespace", () => {
        const lazyHandler = jest.fn((compomintNs) => {
        ...
        const renderingFunc = compomint.template(
          "lazy-eval-compomint-ns",
          "<div>### data.handler(compomint) ##</div>"
        );
        const component = renderingFunc({ handler: lazyHandler });
    ...
    describe("escape (##-)", () => {
      ...
      it("should access able compomint namespace", () => {
        const renderingFunc = compomint.template(
          "escape-compomint-ns",
          "<div>##- compomint.configs.debug ? '<script>alert(\"xss\")</script>' : 'Safe' ##</div>"
        );
        const component = renderingFunc({});
        expect(component.element.innerHTML).toBe("Safe");
        compomint.configs.debug = true;
        const component2 = renderingFunc({});
        expect(component2.element.innerHTML).toBe(
          '&lt;script&gt;alert("xss")&lt;/script&gt;'
        );
      });
    });
    ...
    describe("evaluate (##)", () => {
      it("should execute arbitrary JavaScript code", () => {
        const renderingFunc = compomint.template(
          "eval-basic",
          "## let x = 5; let y = 10; ##<div>Result: ##= x + y ##</div>"
        );
        const component = renderingFunc({});
      ...
      it("should allow flow control (loops)", () => {
        const renderingFunc = compomint.template(
          "eval-loop",
          "<ul>## for(let i = 0; i < data.items.length; i++) { ##<li>Item ##= i+1 ##: ##= data.items[i] ##</li>## } ##</ul>"
        );
        const component = renderingFunc({ items: ["A", "B", "C"] });
        expect(component.element.innerHTML).toBe(
          "<li>Item 1: A</li><li>Item 2: B</li><li>Item 3: C</li>"
        );
      });
      ...
      it("should allow flow control (conditionals)", () => {
        const renderingFunc = compomint.template(
          "eval-if",
          "## if (data.isAdmin) { ##<span>Admin</span>## } else { ##<span>User</span>## } ##"
        );
      ...
      `
        ...
        expect(component.element.innerHTML).toBe(
          "Data: 123 | Status: true | Comp: Set In Eval | I18n: Hello"
        );
      ...
      it("should access able compomint namespace", () => {
        const renderingFunc = compomint.template(
          "eval-compomint-ns",
          "## compomint.configs.debug = true; ##<div>Debug Mode: ##= compomint.configs.debug ##</div>"
        );
        const component = renderingFunc({});
        expect(component.element.innerHTML).toBe("Debug Mode: true");
        expect(compomint.configs.debug).toBe(true);
        compomint.configs.debug = false; // Reset for other tests
      });
    });
  ...
  describe("Rendering (tmpl function)", () => {
    ...
    it("should render with data interpolation (##=)", () => {
      const renderingFunc = compomint.template(
        "render-interp",
        "<div>Hello ##=data.name##! Age: ##=data.age##</div>"
      );
    ...
    it("should execute JavaScript evaluation (##)", () => {
      const renderingFunc = compomint.template(
        "render-eval",
        "<ul>## for(let i=0; i<data.items.length; i++){ ##<li>##=data.items[i]##</li>## } ##</ul>"
      );
    ...
    it("should render an empty template placeholder if data is null/undefined", () => {
      ...
      expect(componentNull.element.dataset.coEmptyTemplate).toBe(
        "render-empty"
      );

      const componentUndef = renderingFunc(undefined);
      expect(componentUndef.element.tagName).toBe("TEMPLATE");
      expect(componentUndef.element.dataset.coEmptyTemplate).toBe(
        "render-empty"
      );
    });
  ...
  describe("Lazy Execution Features", () => {
    ...
    it("should execute lazy evaluation blocks (###) after render", () => {
      ...
      const renderingFunc = compomint.template(
        "lazy-eval",
        "<div>### data.lazyFunc(data.val) ###</div>"
      );
    ...
    it("should assign named elements (data-co-named-element - variable)", () => {
      const renderingFunc = compomint.template(
        "named-el",
        '##let myButton = "myButton";##<button data-co-named-element="##:myButton##">Click</button>'
      );
      const component = renderingFunc({});
      expect(component.myButton).toBeDefined();
      expect(component.myButton.tagName).toBe("BUTTON");
    ...
    it("should assign element references (data-co-element-ref)", () => {
      ...
      const renderingFunc = compomint.template(
        "el-ref",
        '<input data-co-element-ref="##:myInput##"></input>###data.lazyFunc(myInput);##'
      );
    ...
    it("should execute element load functions (data-co-load)", () => {
      ...
      const renderingFunc = compomint.template(
        "el-load",
        "<div data-co-load=\"##:data.loadFunc::'custom'##\">Load</div>"
      );
    ...
    it("should insert elements (##%)", () => {
      const renderingFunc = compomint.template(
        "el-insert",
        "<div>Before ##% data.content ## After</div>"
      );
      ...
      expect(component.element.innerHTML).toBe(
        "Before <span>Inserted</span> After"
      );

      // Elements String
      component = renderingFunc({ content: "<i>Italic</i><b>Bold</b>" });
      expect(component.element.innerHTML).toBe(
        "Before <i>Italic</i><b>Bold</b> After"
      );
      ...
      items[2].textContent = "Three";
      component = renderingFunc({ content: items });
      expect(component.element.innerHTML).toBe(
        "Before One<span>Two</span><i>Three</i> After"
      );
      ...
      expect(component.element.innerHTML).toBe(
        "Before <section>Inner</section> After"
      );
      expect(innerComponent.parentComponent).toBe(component);
    });
    ...
    describe("Event Handling (data-co-event)", () => {
      ...
      it("should handle custom data (object)", () => {
        ...
        const renderingFunc = compomint.template(
          "event-custom",
          '<button data-co-event="##:data.handler:: {id: 100} ##">Data</button>'
        );
        const component = renderingFunc({ handler });
        component.element.click();
        expect(handler).toHaveBeenCalledWith(
      ...
      it("should handle custom data (variable)", () => {
        ...
        const renderingFunc = compomint.template(
          "event-custom",
          '##let customData = {id: 101};##<button data-co-event="##:data.handler::customData ##">Data</button>'
        );
        const component = renderingFunc({ handler });
        component.element.click();
        expect(handler).toHaveBeenCalledWith(
  ...
  describe("Component Methods", () => {
    it("_id should have a unique _id", () => {
      ...
      expect(component1.element.outerHTML).toBe(
        `<span id="${component1._id}">ID</span>`
      );
      expect(component2.element.outerHTML).toBe(
        `<span id="${component2._id}">ID</span>`
      );
    });
    ...
    it("replace() should replace the element", () => {
      const render1 = compomint.template(
        "component-replace1",
        "<span>One</span>"
      );
  ...
  describe("compomint.tools Utilities", () => {
    describe("escapeHtml", () => {
      it("should escape HTML characters", () => {
        expect(tools.escapeHtml.escape('<div class="foo">')).toBe(
          "&lt;div class=&quot;foo&quot;&gt;"
        );
        expect(tools.escapeHtml.escape("'`&")).toBe("&#x27;&#x60;&amp;");
      });

      it("should unescape HTML entities", () => {
        expect(tools.escapeHtml.unescape("&lt;div&gt;")).toBe("<div>");
        expect(tools.escapeHtml.unescape("&#x27;&#x60;&amp;")).toBe("'`&");
      });
    });
    ...
    describe("addTmpl / addTmpls", () => {
      ...
      it("should add templates from an element source", () => {
        const div = document.createElement("div");
        div.innerHTML =
          '<template id="t1">T1</template><script type="text/template" id="t2">T2</script><template id="t3">T3</template><script type="text/compomint" id="t4">T4</script>';
      ...
      it("should remove inner templates if requested", () => {
        const div = document.createElement("div");
        div.innerHTML =
          '<template>T1</template><script type="text/template">T2</script><template id="t3">T3</template><script type="text/compomint">T4</script>';
        compomint.addTmpls(div, true); // removeInnerTemplate = true
        expect(div.querySelector("template")).not.toBeNull();
        expect(
          div.querySelector('script[type="text/template"]')
        ).not.toBeNull();
        expect(
          div.querySelector('script[type="text/compomint"]')
        ).not.toBeNull();
    ...
    describe("i18n", () => {
      ...
      it("should preserve array structure in i18n", () => {
        ...
        expect(compomint.i18n.menu3.test1.test2.title()).toBe("Title");
        ...
        expect(compomint.i18n.menu3.test1.test2.items[0].home()).toBe("홈1");
        expect(compomint.i18n.menu3.test1.test2.items[0].about()).toBe("소개1");
        expect(compomint.i18n.menu3.test1.test2.items[1].home()).toBe("홈2");
        expect(compomint.i18n.menu3.test1.test2.items[1].about()).toBe("소개2");
      ...
      it("should allow loop operations on i18n array", () => {
        ...
        document.documentElement.lang = "en";
        // Test individual array items are accessible
        expect(compomint.i18n.categories.list[0].tech()).toBe("Tech");
        expect(compomint.i18n.categories.list[1].science()).toBe("Science");
        expect(compomint.i18n.categories.list[2].arts()).toBe("Arts");

        document.documentElement.lang = "ko";
        expect(compomint.i18n.categories.list[0].tech()).toBe("기술");
        expect(compomint.i18n.categories.list[1].science()).toBe("과학");
        expect(compomint.i18n.categories.list[2].arts()).toBe("예술");
      });
    });
    ...
    describe("genElement", () => {
      ...
      it("should create an element with data-*", () => {
        const el = tools.genElement("div", {
          "data-value": "test",
          "data-id": 123,
          "data-next-str": "next string",
        });
        expect(el.tagName).toBe("DIV");
      ...
      it("should create an element with dataset", () => {
        const el = tools.genElement("div", {
          dataset: { value: "test", id: 123, nextStr: "next string" },
        });
        expect(el.tagName).toBe("DIV");
        expect(el.dataset.value).toBe("test");
        expect(el.dataset.id).toBe("123");
